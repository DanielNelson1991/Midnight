...
public class DoorScript : MonoBehaviour {
	...
	public int keyID;					// What is the Key ID for this door. ID is defined withing the Item Database

	AudioSource audioSource;			// Make a reference to an audio source component 
	Inventory inventory;                // Make a reference to the Inventory
	...
	public void CheckDoor()
	{
		...
		if(isDoorLocked)
		{
			...
			if(inventory.InventoryContains(keyID))
			{
				...
				chara = GameObject.FindGameObjectWithTag("Player").GetComponent<CharacterScript>();
				chara.CheckTaskSystem();
